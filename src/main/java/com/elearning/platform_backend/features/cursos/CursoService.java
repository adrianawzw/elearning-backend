...
import com.elearning.platform_backend.features.usuarios.DocenteRepository;
...
public class CursoService {
    ...
    // Método Post para crear un nuevo Curso usando Transacional para verificar que el Docente exista
    @Transactional
    ...
    public List<CursoReaderDTO> findAll(){
        if(cursoRepository.count() == 0) throw new ResponseStatusException(HttpStatus.NO_CONTENT, "No hay cursos registrados");
    ...
    public void deleteById(Long id){
        if(!cursoRepository.existsById(id)) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Curso no existe");
        cursoRepository.deleteById(id);
    }

    // Método GET para obtener Curso por Nombre
    public CursoReaderDTO findByTitulo(String titulo){
        Curso curso = cursoRepository.findByTitulo(titulo).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Curso no encontrado"));
    ...
    public CursoReaderDTO update(Long id, CursoUpdateDTO dto) throws Exception {
        ...
        Curso curso = cursoRepository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Curso no existe"));
        Docente docente = docenteRepository.findById(dto.docenteID()).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Docente no existe"));
